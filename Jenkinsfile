  ...
  tools { 
   ...
   stages{
    stage('CompileandRunSonarAnalysis') {
            steps {	
		sh "mvn clean verify sonar:sonar -Dsonar.projectKey=asgbuggywebapp100 -Dsonar.organization=asgbuggywebapp100 -Dsonar.host.url=https://sonarcloud.io -Dsonar.token=${SONAR_TOKEN}"
			}
        } 
